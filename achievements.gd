...
func _ready() -> void:
	...
	for i in range(Globals.ACHIEVEMENTS.size()):
		...
		# Отступы для 3 колонок
		var x := 40.0 + float(col) * 210.0
		var y := 70.0 + float(row) * 75.0

		# Сдвигаем иконку влево и текст вправо (максимальное расстояние)
		UI.make_icon(root, "star" if done else "hud_heart_empty", Vector2(x - 30, y + 2), 26.0)
		...
		progress.position = Vector2(x + 40.0, y + 26.0)
		progress.size = Vector2(160, 18)
	...
	var back := UI.make_button(root, L.t("back"), Vector2(250, 420), Vector2(140, 34), 15)
